...
messages = {
    ...
    'osmand': 'OSMAND,123456789012345,2020-01-01 00:00:00,59.93211887,30.33050537,0.0'
}
...
def load_ports():
    """Load protocol ports from traccar.xml instead of Java config"""
    ports = {}
    dir = os.path.dirname(os.path.abspath(__file__))
    traccar_xml_path = dir + '/../traccar/traccar.xml'

    try:
        tree = xml.etree.ElementTree.parse(traccar_xml_path)
        root = tree.getroot()

        # Extract ports from entries like: <entry key='osmand.port'>5055</entry>
        for entry in root.findall('entry'):
            key = entry.get('key', '')
            if key.endswith('.port'):
                protocol = key.replace('.port', '')
                port = int(entry.text) if entry.text else None
                if port:
                    ports[protocol] = port

        if debug:
            print(f'\nPorts loaded from traccar.xml: {ports!r}\n')
    except Exception as e:
        print(f'Error loading ports from traccar.xml: {e}')
...
if __name__ == "__main__":
    ...
    all = set(ports.keys() - ["web"])
    protocols = set(messages.keys())

    print(f'Total: {len(all)}')
    print(f'Missing: {len(protocols - all)}')
